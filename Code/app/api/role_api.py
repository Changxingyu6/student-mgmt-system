...
@require_roles()  # 只有 admin 能访问
def get_role_detail(role_id: int, request: Request, db: Session = Depends(get_db)):
    """根据ID获取角色详情"""
...
@require_roles()  # 只有 admin 能访问
def update_role(role_id: int, request: Request, role_request: RoleUpdateRequest, db: Session = Depends(get_db)):
    """更新角色信息"""
    try:
