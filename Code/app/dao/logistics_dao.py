...
def logistics_query_dao(logistics_id, db: Session):
    data = db.query(Logistics).filter(Logistics.logistics_id == logistics_id).first()
    if not data:
        return False
...
def logistics_query_by_order_dao(order_id, db: Session):
    data = db.query(Logistics).filter(Logistics.order_id == order_id).first()
    if not data:
        return False
    return {k: v for k, v in data.__dict__.items() if not k.startswith('_')}
...
def logistics_update_dao(logisticsdata: dict, db: Session):
    try:
        logistics = db.query(Logistics).filter(Logistics.logistics_id == logisticsdata.get("logistics_id")).first()
        if not logistics:
            return False
