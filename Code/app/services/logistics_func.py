...
def logistics_insert_func(logisticsdata, db):
    ...
    return logistics_dao.logistics_insert_dao(data, db)

# 更新方法
def logistics_update_func(logisticsdata, db):
    data = logisticsdata.model_dump()
    return logistics_dao.logistics_update_dao(data, db)

# 删除方法
def logistics_delete_func(logistics_id, db):
    return logistics_dao.logistics_delete_dao(logistics_id, db)
